...
<div class="ml-auto offset-md-5 col-md-7 p-t-20" id="optionkurir_jasa">
    <div class="login_title form-customer">
        <h6 class="p-b-24 bo20 color0"><i class="fa fa-chevron-circle-right"></i> <u>Metode Pengiriman</u></h6>
    </div>
    <div class="login_form row form-customer">
        <div class="col-md-8 form-group">
            <select class="form-control" id="pilih_jasa" name="jasa" required="">
                <option value=""> Pilih Kurir</option>
                <option value="cod"> COD</option>
                <option value="jasa"> JASA</option>
            </select>
        </div>
        <div class="col-md-12" id="datakurirAlert_jasa"></div>
        <div class="col-md-12">
            <table class="table bo9 bg5">
                <tbody id="datakurir_jasa">
                </tbody>
            </table>
        </div>
    </div>
...
<!-- jasa pengiriman -->
<div class="ml-auto offset-md-5 col-md-7 p-t-20" id="optionkurir_cod">
    <div class="login_title form-customer">
        <h6 class="p-b-24 bo20 color0"><i class="fa fa-chevron-circle-right"></i> <u>Jasa Pengiriman Barang COD</u></h6>
    </div>
    <div class="login_form row form-customer">
        <div class="col-md-8 form-group">
            <select class="form-control" id="kurir_cod" name="kurir_cod" required="">
                <option value=""> Pilih Kurir</option>
                <?php foreach ($kurir as $key) {
                    if($key['status'] == 'Active'){?>
                        <option value="<?=$key['idcod']?>"><?=$key['lokasi']?></option>
                    <?php } } ?>
                </select>
            </div>
            <div class="col-md-12">
                <table class="table bo9 bg5">
                    <tbody id="datakurir_cod">
                    </tbody>
                </table>
            </div>
        </div>
    ...
    <div class="ml-auto offset-md-5 col-md-7 p-t-30" id="optionvoucher">
        <div class="login_title form-customer">
            <h6 class="p-b-24 bo20 color0"><i class="fa fa-chevron-circle-right"></i> <u>Kode Voucher</u><br>
                <a class="text-info" href="<?php echo base_url('pages/voucher');?>" target="_blank">Tidak memiliki voucher ? Cek sekarang!</a>
            </h6>
        </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $('#loadingpage').hide();
        $('#loadingbutton').hide();
        $('#optionkurir').hide(300);
        $('#optionkurir_cod').hide(300);

        $('#pilih_jasa').change(function() {
            if($(this).val() == ""){
                $('#optionkurir').hide(300);
                $('#optionkurir_cod').hide(300);
            }else if($(this).val() == "cod"){
                $('#optionkurir').hide(300);
                $('#optionkurir_cod').show(300);
            }else{
                $('#optionkurir').show(300);
                $('#optionkurir_cod').hide(300);
            }
        });

        $("#kurir_cod").on("change", function cost(e) {
            e.preventDefault();
            var id = $('option:selected', this).val();
            var $op = $("#datakurir_cod");
            $.ajax({
                url: "<?php echo base_url('cart/getKurirCod'); ?>",
                dataType: "JSON",
                method: "POST",
                data: {id:id},
                success: function (data) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ht += '<tr><td><div class="form-check"><label class="form-check-label"><input type="radio" class="form-check-input ongk_" data-fx="'+data[i]['ongkir']+'" data-fx1="'+data[i]['name']+'" value="' + data[i]['name'] + '-' + data[i]['lokasi'] + '" required>';
                        ht += ' ' + data[i]['name'] +' - ' + 'Wilayah' + ' '+ data[i]['lokasi'] +number_idr(data[i]['ongkir']);

                        ht += '</label></div></td></tr>';   
                    }
                    $op.html(ht);
                    $('.ongk_').click(function() {            
                        set_ongkir_edit($(this).data('fx'),$(this).data('fx1'));
                    });
                }
            });
        });
        ...
        // function set_Cod(idr,name){
        //     $("#loadingpage").removeClass("loadingfull");
        //     $('#loadingpage').hide();
        //     var idr = number_idr(parseInt(idr));
        //     var idr = 'Rp. ' + idr + '';
        //     var x = '<div class="alert alert-success" role="alert">Ongkos Kirim : '+idr+'</div>';
        //     $('#datakurir_cod').html(x);  
        //     set_ongkir_edit(idr,name);
        // }
        ...
        function set_ongkir_edit(idr,shiping) {
            $('#loadingpage').show();
        ...
        $.ajax({
            ...
            success: function (data) {
                if(data.desa == null){
                    ...
                    function (isConfirm) {
                        if (isConfirm) {
                            window.location.href="<?php echo base_url('pages/update_account/'.encryption($_SESSION['iduser']).'');?>";
                        }
                    });
                }
                ...
                if(data.kecamatan == null){
                    $('#kecamatan').append('<option value="" selected="selected">Pilih Kecamatan</option>');
                }else{
